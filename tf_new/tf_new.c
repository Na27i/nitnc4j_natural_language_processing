...
int main(int argc, char **argv) {
	...
	for (filenum = 1; filenum <= 100; filenum++)
	{
		...
		sprintf(outputFileName, "%s%03d%s", "..\\data\\tf_new_", filenum, ".txt");
		...
		for (; node; node = node->next)
		{
			...
			if (node->posid > 3 && node->posid < 10)//記号
				...
				Flag = 1;
			else if (node->posid == 36 || node->posid == 50)//サ変接続
				Flag = 1;
			else if (node->posid == 48)//数字
				Flag = 1;
			else if (node->length < 3 && node->char_type == '\x5')//2文字以下の英語
				Flag = 1;
			// ↑　改良点　↑
